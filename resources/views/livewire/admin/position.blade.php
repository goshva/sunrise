  ...
  <div class="name">
    ...
    <div class="position_level">
        ...
        $levels = [];
            ...
            if($pos_level != 0 && count($levels) != 0){

            $pos_level = $pos_level / count($levels);
            }
        @endphp
     <div class="position_level_content">
         <span>
            @if (round($pos_level) == 1)
            Осведомленность
            @elseif (round($pos_level) == 2)
            Знание
            @elseif (round($pos_level) == 3)
            Опыт
            @elseif (round($pos_level) == 4)
            Мастерство
            @elseif (round($pos_level) == 5)
            Эксперт
            @else
            Не определен
            @endif
...
<div class="position-content_sector-wrap" id="sector{{  $position->id }}">
        ...
        <div style="display: grid;grid-template-columns: 2fr 1.5fr 1.5fr 0.1px;
    gap: 20px;
    padding: 20px 0;
    text-align: start;
    ...
    <!--таблица-->
    <div class="position-content_section" id="position-content_section{{$position->id}}" style="display: flex;justify-content:end">
        <div style="display: grid;
    flex-direction: column;
    width: 80%;
    grid-template-columns: 1fr 1.5fr 1.5fr;
    justify-content: end;">
            @foreach ($position->competetions as $key => $competetion)
            <div></div>
                        <div style="display: flex;flex-direction:column; gap: 10px; margin-top: 10px; width:300px;">
          ...
          <div>
               @if($competetion->indicators !== null && $competetion->indicators !== [])
                 @foreach ($competetion->indicators as $indicator)
                     @foreach($competetion->levels as $competetionLevel)
                         @if($competetionLevel->level == $indicator->level && $position->id == $competetionLevel->position_id)
...
<div class="position-popup  @if ($this->addCompetationPositions !== []) active @endif" id="popup3">
    ...
    <div class="position-popup_content">
        ...
        <select id="addLevelToAddCompetations" class="position-popup_select">
            <option selected class="position-popup_option">Добавить уровень</option>
          {{--@foreach ($all_levels as $level_item)
            <option value="{{ $level_item->id }}" class="position-popup_option">{{ $level_item->name }}</option>
                
            @endforeach--}}
            <option value="1">Осведомленность
</option>
            <option value="2">Знания</option>
            <option value="3">Опыт</option>
            <option value="4">Мастерство</option>
   ...
   const addLevelToAddCompetations = document.getElementById("addLevelToAddCompetations").value;
   
if(competetionName !== '' && competetionName !== ' ' && competetionName !== null && addPositionsToAddCompetations !== 'Добавить должности' && addCompetetionForIndicator !== null && addLevelToAddCompetations !== 'Добавить компетенции' && addLevelToAddCompetations !== null) {
    window.livewire.emit('addCompetetion',competetionName);
     ...
     window.livewire.emit("addLevelToAddCompetations", addLevelToAddCompetations);
} else {
...
<div class="position-popup  " id="popup4">
    ...
    <div class="position-popup_content">
        ...
        <input id="addIndicatorName" class="position-popup_input" type="text">
            <select class="position-popup_select" name="" id="competetion_level_test">  
                <option disabled>Уровень</option>
                <option  value="1">Осведомленность</option>
                <option value="2">Знание </option>
                <option value="3">Опыт</option>
                <option value="4">Мастерство</option>
                <option  value="5">Эксперт</option>
            </select>
        ...
        <script>
            function addIndicatorDB(){
                ...
                const competetion_level_test = document.getElementById('competetion_level_test').value
    ...
    window.livewire.emit('addIndicator',indicatorName,addCompetetionForIndicator,competetion_level_test)
