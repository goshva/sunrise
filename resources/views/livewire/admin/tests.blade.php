  ...
  @endif
    <div class="creating-info">  
        ...
        <div class="creating-sector">
            <h1 class="creating-sector_title">Составление теста</h1>
            <a wire:click.prevent="saveAndQuit" href="" class="creating-sector_btn">Завершить</a>
        </div>
        <p class="creating-info_subtitle">шаг 1</p>
    ...
    <div class="creating-content">
        ...
        <div class="create-content">
            ...
            <select class="create-content_select" name="" id="competetion_level_test">  
            ...
            <select class="create-content_select" id="create-content_select">
                <option @if (!$this->competetion) selected @endif  class="create-content_option">Компетенция</option>
                {{-- {{ dd(request()->all()) }} --}}
                @foreach ($this->competetions as $competetion)
                <option 
                @if ($this->competetion)
                @if ($this->competetion->id == $competetion->id)
                   selected
                 @endif
                 @endif 
                ...
                @endforeach
                 ...
                 @foreach ($this->competetion->indicators->where("level", $this->selectedLevel) as $indicator)
        ...
        <div class="writing-content" id="test-writing-content">
            ...
            <div class="writing-question">
                <h2 class="writing-question_title">Введите вопрос на уровень : {{ $this->questionLevel }}</h2>
                <label class="writing-question_label">Вопрос</label>
                @if ($this->goBackPage> 0)
                <input value="{{ $this->question }}" id="update-test-question" type="text"  class="writing-question_input" >
                <script>
            ...
            update_test_question.addEventListener("change",()=>{
                ...
                @else
                <input wire:model="question" type="text" id="test-question" class="writing-question_input" >
                @endif
            </div>
            ...
            <div class="writing-question">
                ...
                <div class="test-questions">
                    ...
                    @endif @endif id="test-question-input-3" class="writing-question_input" >
                            <input class="test-question-radio"  type="radio" id="test-question-radio-3" name="radio">
                        </div>
                    </div>
...
const competetion_level_test = document.getElementById('competetion_level_test');
competetion_level_test.addEventListener("change", ()=>{
    window.livewire.emit('competetionLevel',competetion_level_test.value)
})
...
function addNewTestData (){
    ...
    // Get references to the loader and input elements
